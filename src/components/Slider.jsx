...
import { vertexShader } from "@/data/shaders";
import { fragmentShader } from "@/data/shaders";

const Slider = ({
  scrollIntensity = 0,
  targetScrollIntensity = 0,
  maxScrollIntensity = 1.0,
  scrollSmoothness = 0.5,
  scrollPosition = 0,
  targetScrollPosition = 0,
  scrollPositionSmoothness = 0.05,
  isMoving = false,
  movementThreshold = 0.001,
  isSnapping = false,
  stableCurrentIndex = 0,
  stableNextIndex = 1,
  isStable = false,
  titleHidden = false,
  ...
  useEffect(() => {
    ...
    const calculatePlaneDimensions = () => {
      ...
      const widthFactor = window.innerWidth < 900 ? 0.8 : 0.55;
    ...
    scrollIntensity,
    targetScrollIntensity,
    maxScrollIntensity,
    scrollSmoothness,
    scrollPosition,
    targetScrollPosition,
    scrollPositionSmoothness,
    isMoving,
    movementThreshold,
    isSnapping,
    stableCurrentIndex,
  ...
  return (
    <div
      className="container relative w-screen h-svh overflow-hidden z-10"
      ref={sliderRef}
    >
      <div className="project-title-container fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1/2 aspect-video">
        <div
          id="project-link"
          className="w-full h-full flex items-end text-white object-cover"
        >
          <div className="project-title-mask relative w-full overflow-hidden mask">
            <p
              id="project-title"
              className="opacity-0 lg:opacity-100 block relative translate-y-0 text-white text-center transition-transform duration-500 ease-in-out pr-1 uppercase select-none"
            >
              Title 1
            </p>
          </div>
